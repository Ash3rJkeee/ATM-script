...
class HeatSource:
    ...
    def __init__(
        ...
        # определение вычисляемой подпитки
        if (self.g1 is not None) and (self.g2 is not None):
            self.gp_calc = Parameter()
            self.gp_calc.val = self.g1.val - self.g2.val        # подпитка на основе разницы расходов
            self.gp_calc.last_dt = min(self.g1.last_dt, self.g2.last_dt)    # время обновления определять по самому
                                                                            # старому
            self.gp_calc.descr = "gp_cal"

            # если хоть один из расходов устарел, то разница расходов тоже устарела
            if self.g1.expired or self.g2.expired:
                self.gp_calc.expired = True
...
def find_param_by_id(dumps: json, id: int):
    """Ищет значение параметров в json по его id """
    param = Parameter()
    for dump_object in dumps:
        if dump_object["id"] == id:
            param.val = dump_object["val"]
            param.last_dt = dump_object['last_dt']
            param.expired = bool(dump_object['expired'])
            param.physicname = dump_object['physicname']
            param.descr = dump_object['descr']
